package Lib.Auto.JNL_Payment_Report;
...
public class JNLPaymentReport extends HttpServlet implements Serializable {
	private static final long serialVersionUID = 1L;

	private static Logger log4jLogger = Logger.getLogger(JNLPaymentReport.class);
	...
	public void performTask(HttpServletRequest request,
		...
		try {
			...
			if(flag.equals("PaymentNo"))
			{
				...
				log4jLogger.info("==========Payment Number Search For Payment Report========"+flag);
				...
				indexPage ="/JNL_Payment_Report/search_nmvc.jsp?check=ok&flag="+flag+"";
			...
			{				
				log4jLogger.info("=== Inside Print JNLPaymentReport ===");
    			...
    			String namedQuery=ReportQueryUtill.JNL_Payment_Report;
				...
				parameters.put("ReportTitle",ReportQueryUtill.JNL_Payment_Title);   				
				...
				if(txtreporttype.equals("PmtNoWise"))  {
				     ...
				     inputStream = getServletContext().getResourceAsStream("/Report/JNL_PaymentReceipt_Report.jasper");
				
				}else  {
	    	      	
					inputStream = getServletContext().getResourceAsStream("/Report/JNL_Payment_Report.jasper");
		        ...
		        response.setHeader("Content-Disposition", "attachment;filename="+ReportQueryUtill.JNL_Payment_Title+".pdf");
