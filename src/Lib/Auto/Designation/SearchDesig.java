...
public class SearchDesig extends HttpServlet implements Serializable {
...
singlecodecheck code=new singlecodecheck();
	...
	public void performTask(
		...
		try {
			...
			SQLString ="select * from designation_mas where 2>1 and Desig_name like '" + nm + "%' order by Desig_code";
  st=con.createStatement();
  rs = st.executeQuery(SQLString);
    while(rs.next()){
             
		   String f1=rs.getString("Desig_Code");
           String f2=rs.getString("Desig_Name");
		...
		ob.setAl(ser);
	 getServletConfig().getServletContext().getRequestDispatcher("/Desig/search.jsp?check=ok&nam="+nm+"").forward(request, response);
