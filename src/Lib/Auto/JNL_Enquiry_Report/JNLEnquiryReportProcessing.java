package Lib.Auto.JNL_Enquiry_Report;
...
public class JNLEnquiryReportProcessing extends HttpServlet implements Serializable {
	private static final long serialVersionUID = 1L;

	private static Logger log4jLogger = Logger.getLogger(JNLEnquiryReportProcessing.class);
	...
	public void performTask(HttpServletRequest request,
		...
		try {
			...
			if(flag.equals("Sup"))
			{
				...
				ob1.setAdd2("EnquiryReport");
				InvArrylist=ss1.getInvSearchOrdNoSup(ob1);				
																						
				request.setAttribute("search", InvArrylist);
				indexPage ="/JNL_Enquiry_Report/search_nmvc.jsp?check=ok&flag="+flag+"";
			...
			if(flag.equals("Print"))
			{
				...
				log4jLogger.info("=== Inside Print JnlEnquiry ===");
				
				String ordno=null,strsql1=null;
				
				ordno=request.getParameter("ordno");
	            strsql1="and enq_no='"+ordno+"'";				
				
				con=SessionHibernateUtil.getSession().connection();
	     		
    	      	Map parameters = new HashMap();       	     
    			String namedQuery=ReportQueryUtill.JNL_Enquiry_Report;
				...
				parameters.put("ReportTitle",ReportQueryUtill.JNL_Enquiry_Title);   				
				parameters.put("All_Query",sb.toString());				
				log4jLogger.info("SQL QUERY: " + sb);						
				 				
  	      	
				InputStream inputStream = getServletContext().getResourceAsStream("/Report/JNL_Enquiry_Report.jasper");
		        ...
		        response.setHeader("Content-Disposition", "attachment;filename="+ReportQueryUtill.JNL_Enquiry_Title+".pdf");
