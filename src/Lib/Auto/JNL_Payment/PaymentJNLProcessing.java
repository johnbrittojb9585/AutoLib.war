...
public class PaymentJNLProcessing extends HttpServlet implements Serializable {
	private static final long serialVersionUID = 1L;

	private static Logger log4jLogger = Logger.getLogger(PaymentJNLProcessing.class);
	...
	public void performTask(HttpServletRequest request,
		...
		try {
			...
			if (flag.equals("new")) {
				...
				session.setAttribute("JNLPayment", ob);
				indexPage = "/JNL_Payment/index.jsp";
			...
			if(flag.equals("INVOICE"))
			{
				...
				log4jLogger.info("==========Invoice No Search========"+flag);
				...
				PmtArrylist=ss.getPaymentInvSearch(ob);				
																						
				request.setAttribute("search", PmtArrylist);
				indexPage ="/JNL_Payment/search_nmvc.jsp?check=ok&flag="+flag+"";
				dispatch(request, response, indexPage);
			}
			...
			OrdJNLArrylist=ss.getPaymentSelectedInvoice(ob);
			
			
			session.setAttribute("JNLPayment",OrdJNLArrylist);
			session.setAttribute("JNLPaymentSize",OrdJNLArrylist.size());
			...
			if (flag.equals("Save")) {
				...
				add2=request.getParameter("add2");			
				 ...
				 List OrderSave=null;	
				 OrderSave=ss.getPaymentJNLMasSave(ob);
			...
			if(OrderSave.size()==1){
				...
				session.setAttribute("JNLPayment",saveDetail);
				session.setAttribute("JNLPaymentSize",saveDetail.size());
				
				log4jLogger.info("----Update Check For Payment For Journal-------"+OrderSave.size());
				indexPage ="/JNL_Payment/index.jsp?check=UpdateCheck";
				
			}else if(OrderSave.size()==2)  {
				
				log4jLogger.info("----Already Paid for this Invoice-------"+OrderSave.size());
				indexPage ="/JNL_Payment/index.jsp?check=PaidCheck";
					...
					List<Object> saveDetail = new ArrayList<Object>();					
					 ...
					 for (int i=0; i<len; i++) {				 
						 ...
						 JnlPaymentBean Paymentbean=new JnlPaymentBean();
				 ...
				 ss.getPaymentJNLDetailsUpdate(saveDetail);	 
				 indexPage ="/JNL_Payment/index.jsp?check=SaveSuccess";	
			...
			if (flag.equals("search")) {
				...
				List OrderView=null;
				OrderView=ss.getPaymentJNLDetailsSearch(payment_no);
				 
				if(OrderView.size()>0)  {
				  session.setAttribute("JNLPayment",OrderView);
				  session.setAttribute("JNLPaymentSize",OrderView.size());				
				
				indexPage ="/JNL_Payment/index.jsp";				
				} else {
					indexPage ="/JNL_Payment/index.jsp?check=DeleteFail";							
				}				
				dispatch(request, response, indexPage);				
			}
			...
			if (flag.equals("delete")) {
				...
				if(OrderView.size()>0)
				{							
					ss.getPaymentJNLDelete(payment_no);					
					indexPage ="/JNL_Payment/index.jsp?check=DeletedSuccess";
			...
			if (flag.equals("update")) {
				...
				add2=request.getParameter("add2");			
				 ...
				 ss.getPaymentJNLDelete(payment_no);  //  Delete Payment Master
				...
				indexPage ="/JNL_Payment/index.jsp?check=UpdateSuccess";
